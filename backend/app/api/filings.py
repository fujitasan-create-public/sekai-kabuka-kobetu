...
_tdnet_cache: dict[str, tuple[float, list[dict]]] = {}


async def _tdnet_parse_page(
    client: httpx.AsyncClient,
    sem: asyncio.Semaphore,
    url: str,
    code5: str,
) -> tuple[list[dict], int]:
    """1ページをフェッチしてマッチ行と総件数を返す。"""
    async with sem:
        try:
    ...
    for row in table.find_all("tr"):
        ...
        results.append({
            ...
            "url": f"{TDNET_BASE}/{href}" if href else "",
            "source": "tdnet",
        })
...
async def _tdnet_fetch_date(
    client: httpx.AsyncClient,
    sem: asyncio.Semaphore,
    date_str: str,
    code5: str,
) -> list[dict]:
    """指定日の全ページをチェックし決算短信行を返す。"""
    url1 = f"{TDNET_BASE}/I_list_001_{date_str}.html"
    filings, total = await _tdnet_parse_page(client, sem, url1, code5)

    if total > 100:
        max_page = min((total + 99) // 100, 8)
        extra = await asyncio.gather(*[
            _tdnet_parse_page(client, sem, f"{TDNET_BASE}/I_list_{p:03d}_{date_str}.html", code5)
...
def _tdnet_target_dates(today: date) -> list[str]:
...
async def _fetch_tdnet_filings(sec_code: str) -> list[dict]:
    cached = _tdnet_cache.get(sec_code)
    if cached and monotonic() - cached[0] < _EDINET_CACHE_TTL:
        return cached[1]

    code5 = sec_code + "0"
    today = date.today()
    target_dates = _tdnet_target_dates(today)

    sem = asyncio.Semaphore(25)
    async with httpx.AsyncClient(headers=_TDNET_UA) as client:
        all_results = await asyncio.gather(*[
            _tdnet_fetch_date(client, sem, d, code5)
    ...
    _tdnet_cache[sec_code] = (monotonic(), result)
    return result
...
async def get_filings(ticker: str = Query(...)) -> dict:
    ...
    if is_japan:
        ...
        edinet_task  = asyncio.create_task(_fetch_edinet_filings(sec_code))
        tdnet_task   = asyncio.create_task(_fetch_tdnet_filings(sec_code))
        ...
        return {
            ...
            "tdnet_url": f"https://www.release.tdnet.info/",
            "filings": all_filings,
        }
    else:
